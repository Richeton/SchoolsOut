...
Entity::Entity() : Image()
{
	...
	collisionType = entityNS::COLLISION_TYPE::CIRCLE;
	entityType = entityNS::ENTITY_TYPE::GENERAL;
	health = 100;
}
...
void Entity::setAI(State * aiState)
...
// Called when it collides with an Entity
void Entity::collidedWith(Entity &ent, VECTOR2 &collisionVector, EntityCollection &entityCollection)
{
	// This Entity Type Check
	switch (entityType)
	{
	case entityNS::ENTITY_TYPE::NONE:
		break;

	case entityNS::ENTITY_TYPE::PROJECTILE:
		if (collision)
			SAFE_DELETE(collision);
		collision = new Collision(&entityCollection);
		collision->run(*this, ent, collisionVector);
		break;

	case entityNS::ENTITY_TYPE::RUGBYPLAYERS:
		break;
	...
	// Other Collision Check (Target entity affecting trigger entity)
	...
	default:
		if (ent.collision)
			SAFE_DELETE(ent.collision);
		ent.collision = new Collision(&entityCollection);
		ent.collision->run(ent, *this, collisionVector);
		break;
	}
}
...
bool Entity::collidesWith(Entity &ent, VECTOR2 &collisionVector)
{
	...
	if (collisionType == entityNS::COLLISION_TYPE::CIRCLE && ent.getCollisionType() == entityNS::COLLISION_TYPE::CIRCLE)
		return collideCircle(ent, collisionVector);
	// If both entities are BOX collision
	if (collisionType == entityNS::COLLISION_TYPE::BOX && ent.getCollisionType() == entityNS::COLLISION_TYPE::BOX)
	...
	if (collisionType != entityNS::COLLISION_TYPE::CIRCLE && ent.getCollisionType() != entityNS::COLLISION_TYPE::CIRCLE)
		...
		if (collisionType == entityNS::COLLISION_TYPE::CIRCLE)  // if this entity uses CIRCLE collision
		{
...
//=============================================================================
void Entity::bounce(Entity &ent, VECTOR2 &collisionVector)
{
